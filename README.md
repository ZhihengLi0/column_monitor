...
| 4.8.2 | [v4.8.2](https://github.com/ZhihengLi0/column_monitor/releases/tag/v4.8.2) | **Staleness only for periodically-sampled variables** — per operator guidance, stale-checking now excludes change-based variables (updated only when the value changes, e.g. He flow which sits at 0 when not circulating) and covers only variables written at a fixed interval. Classification confirmed from the data (periodic sensors keep producing records every few seconds/minutes even when stable). Thresholds tuned per sensor to stay spam-free |
